const getLocalStorageFavorites = () => {
  if (localStorage.getItem('favorites') !== null) {
    return JSON.parse(localStorage.getItem('favorites') as string).map((el: string) => Number(el)) as number[];
  } else {
    return null
  }
}
...
export { getLocalStorageFavorites, setLocalStorageFavorite, deleteLocalStorageFavorite, isLocalStorageFavorite }